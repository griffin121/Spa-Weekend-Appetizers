...
export async function getLockSettings() {
    ...
    .maybeSingle();
    return {
        lockHour: data?.lock_hour ?? 14,
        lockMinute: data?.lock_minute ?? 0,
    };
}

export async function setLockSettings(lockHour, lockMinute) {
    await supabase.from("tournament_settings").upsert({
        id: 1,
        lock_hour: lockHour,
        lock_minute: lockMinute,
        updated_at: new Date().toISOString(),
    });
}

export function getNextLockTime(lockHour = 14, lockMinute = 0) {
    const now = new Date();
    const target = new Date(now);
    target.setHours(lockHour, lockMinute, 0, 0);
    if (target <= now) {
...
export function isPastLockTime(lockHour = 14, lockMinute = 0) {
    const now = new Date();
    const todayLock = new Date(now);
    todayLock.setHours(lockHour, lockMinute, 0, 0);
    return now >= todayLock;
}
...
export async function ensureTournament(category, lockHour = 14, lockMinute = 0) {
...
if (!isPastLockTime(lockHour, lockMinute)) return null;
